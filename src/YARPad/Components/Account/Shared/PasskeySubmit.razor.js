...
function buildAccountUrl(path, queryParams) {
...
async function createCredential(headers, signal) {
    const optionsResponse = await fetchWithErrorHandling(buildAccountUrl('Account/PasskeyCreationOptions'), {
        method: 'POST',
        headers,
        signal,
    });
...
async function requestCredential(email, mediation, headers, signal) {
    const optionsResponse = await fetchWithErrorHandling(
        buildAccountUrl('Account/PasskeyRequestOptions', { username: email }),
        {
        method: 'POST',
        headers,
        signal,
